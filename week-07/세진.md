...
- 렌더링을 위해 데이터를 전송할때 Effects를 사용하지 않아도 됩니다. state가 업데이트 되면 commit되며 화면이 바뀌기에 Effects가 필요하지 않습니다.
- user events에 Effects를 사용하지 않아도 됩니다. post요청을 보낸다고 할때 button click 이라는 event handler로 처리할 수 있습니다.

**Effects가 필요할땐 외부 시스템과 동기화 되어야 할때입니다.**

### useEffect를 사용하지 않고 여러 가지 상황을 해결하기
Effects를 사용하지 않는 이유는
- 불필요한 렌더링이 발생하고
- 이전데이터가 보여지는 버그가 발생할 수 있고
- 렌더링에 따라 자식컴포넌트가 오래된 값을 가지고 있는 등

여러 측면에서 비효율적이기 때문입니다.

따라서 아래와 같은 방법으로 해결할 수 있습니다.
- 렌더링 될때 계산할 수 있도록 변수 분리하기
- 이벤트 핸들러가 공유할 로직 분리하기
- prop으로 받아오는 값을 set변수에 저장하고, 렌더링될때 계산하기
- 한번만 실행되어야 하는 함수가 있다면 최상위에 그를 위한 flag 변수 만들기
- clean up 함수 추가하기
