...
> **Summary**
>
> - 상위 component에서 정의한 reducer의 state와 `dispatch` 함수를 하위 component에서 사용할 때, prop 대신 context를 사용해서 전달하여 "prop drilling" 문제를 해결할 수 있다.
> - Reducer와 context를 함께 사용하면 복잡한 state logic을 분리하고 확장성(scalability)을 얻을 수 있다.
> - Reducer와 context를 사용할 때 boilerplate code는 'Provider' component와 custom hook을 활용해서 재사용 할 수 있다.

---

- `useReducer`를 상위 component에서 정의할 때, reducer의 state 및 `dispatch` 함수를 하위 component에서 사용하려면 prop으로 state와 event handler를 전달해야 함
- Prop으로 전달해야 하므로, 이 때에도 "prop drilling" 문제가 발생할 수 있음
- **Context에 reducer의 state 및 `dispatch` 함수를 넣으면** 하위 component들이 state에 접근하거나(read) action을 전달(dispatch actions)하는 event handler를 prop으로 받지 않아도 됨
  ...
  2. State와 `dispatch` 함수를 context로 전달
  ...
  3. 하위 component는 state와 event handler를 prop으로 받는 대신, context를 통해 사용
...
- Reducer와 context를 'Provider' component로 묶어서 하나의 file에서 관리할 수 있음 (e.g. `TasksProvider`)
  ...
  // `useContext(TasksContext)`를 반복해서 작성하지 않기 위한 custom hook
  export function useTasks() {
    return useContext(TasksContext);
  }

  // `useContext(TasksDispatchContext)`를 반복해서 작성하지 않기 위한 custom hook
  export function useTasksDispatch() {
    return useContext(TasksDispatchContext);
  }

  // Context provier를 반복해서 작성하지 않기 위한 provider component
