...
어떤 상태를 깊숙한 자식 컴포넌트에서 사용하려면 props를 계속 전달해야한다. 이는 **props drilling**을 초래할 수 있다. 예를 들어, `App` 안에서 넘어가는 모든 중간 컴포넌트가 props를 사용하지 않지만 **그냥 전달**해야 할 때가 있다.

이렇게 중간 단계를 건너뛰고 전역적으로 사용할 상태를 공급해주기 위해서 `Context`가 나왔다.
...
전달할 값의 **기본값(default value)**을 설정할 수 있다. Provider가 없을 때 이 값이 사용된다.
...
생성한 Context는 컴포넌트에서 useContext로 읽는다. Props를 받지 않고도 깊은 위치에서 값을 읽을 수 있다.
...
useContext도 hook 이니까 컴포넌트 내부에서만 호출해야 한다.
...
기존에는 `LevelContext.Provider` 를 사용해야 했지만 React19 이후로는 그냥 감싸주기만 하면 하위 컴포넌트에서 level값을 사용할 수 있다.

## 같은 컴포넌트에서 사용 및 제공

Context는 상위에서 제공하고 하위에서 사용만 하는 게 아니라, 하위 컴포넌트가 현재 Context 값을 읽고, 그 값을 가공해서 다시 제공하기도 한다.
...
예를 들어 현재 구조에서 전달받은 level을 자동으로 계산해서 다시 내려주기.

또, useContext를 호출하면 가장 가까운 Provider의 값을 자동으로 찾아서 가져온다. 따라서 중간에 어떤 시멘틱 태그나 컴포넌트를 넣어도 괜찮다.

## Context 남용?

Context를 사용하다보면 남용하기 쉬워진다. props drilling 을 해결하기 위해서 Context를 사용하는 것은 좋지만, 먼저 컴포넌트의 구조를 최적화 한다면 해결되는 경우도 있다. Context를 사용하기 전에 이를 먼저 개선해보고 사용하자.

보통은 `테마, 현재유저, 라우팅` 에서 Context를 자주 사용한다.

# Reducer + Context

reducer를 사용할 때도 중간 컴포넌트가 존재할 때 props drilling이 일어날 수 있다.

이때도 Context와 함께 사용하면 이를 해결할 수 있다.

## Reducer와 Context 사용하기

### Context

tasks용 context, dispatch용 context 따로 만들기.
...
이제 자식 컴포넌트는 provider가 전달한 tasks와 dispatch 모두 읽을 수 있다.
...
트리 아래의 각 컴포넌트에서 중간단계를 거치지 않고(props 전달) 필요한 tasks와 dispatch만 가져올 수 있다.

## 파일 분리하기

context, reducer, provider 컴포넌트를 한 파일에서 관리하는 패턴이 있다.
