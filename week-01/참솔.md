...
- JSX는 JavaScript file에 HTML 같은 markup을 작성할 수 있어서 rendering logic과 content를 같은 곳에서 관리할 수 있다.
- Curly braces(`{...}`)를 사용해서 markup 내부에 JavaScript 코드를 동적으로 넣을 수 있다.
- JSX markup의 attribute에 삽입
  ```javascript
...
- JSX markup 내부 text에 삽입
  ...
  ```
- JavaScript object를 삽입한다면 "double curlies"(`{{...}}`) 사용하게 된다.
  ```javascript
  export default function TodoList() {
    return (
      <ul
  ...
  ```
  - HTML과 달리, JSX에서 inline style 설정 시 camel case 사용
