...
가끔씩 state가 같이 관리되고, 변경되어야 하는 상황이 있을 때, 이를 제거하고, 공통 부모 컴포넌트에서 관리하고, prop으로 전달하도록 하는데 이를 **lifting state**라고 한다.
...
위 예시 코드는 `Accordion`이 두 개의 `Panel`을 렌더링하고, `isActive`를 각 `Panel`이 관리하기 때문이 서로 다른 두 `Panel`은 영향을 받지 않는다.

예시를 state가 함께 변경되고 관리하도록(패널은 한개만 펼칠 수 있도록 구현) 해보자. 이 때 lifting state가 필요하다.
    ...
    <>
      <h2>Almaty, Kazakhstan</h2>
      {
        panelList.map((panel)=>(
          <Panel
            title={panel.title}
            isActive={panel.id===activeIindex}
...
`Controlled Component`는 상태가 부모에서 props로 내려오고, 자식은 그 값을 표현만 하거나 이벤트를 부모로 올려보내는 역할만 한다. 부모에 의해 제어되는 컴포넌트.

`Uncontrolled Component`는 부모에게 제어받지 않는, 자식 내부에서 직접 상태를 관리하는 컴포넌트를 말한다.

## Single source of truth

React로 개발할 때는 여러 군데에 state가 흩어질 수 있지만, **각 상태 조각마다 그걸 소유하는 컴포넌트는 하나만** 두는 것이 원칙이다.

공유해야 하는 state를 복제하지 말고, 공통 부모로 끌어올린 뒤 필요한 자식들에 props로 내려보내야 한다.
