...
원시 타입에서는 **값을 바꾼 것이 아닌 새로운 값을 대입해준 것**이기 때문에 업데이트가 반영된 것이고, 객체 타입에서도 불변성 규칙을 지키기 위해서 내부 프로퍼티의 참조값을 바꾸는 것이 아닌 새로운 객체로 교체해줘야 업데이트가 반영된다.
...
복사한 배열이어도 내부 객체를 수정하는 것은 원본에 영향을 미친다. 코드 상으로는 배열의 내부에 있는 것이지만 실제로 메모리에서는 배열 밖에 저장되어있고 배열의 인덱스가 참조하기 때문이다.
